...
def freeze_model_layers(model, freeze):
...
def freeze_aux_model_layers(model):
...
def create_optimizers(model, epochs, hyp, opt):
...
def model_mode(model, opt, rank, cuda, device):
    # DP mode
...
def Hyperparameter_evolution_metadata():
